...
public class CountryArrayManager {
    ...
    public void remove(int index) {
        ...
        this.countryDataArray[this.length] = null;
    }
    ...
    // Hàm sắp xếp không nên làm thay đổi mảng dữ liệu gốc.
    // Vì vậy cấn cấp phát một mảng mới có độ dài là le
    // sellection sort, bubble sort, insertion sort,
    // hoặc dùng Comparable, Comparator của Java.
    ...
    public CountryData[] filterContinent(String continent) {
        ArrayList<CountryData> arrayListContinent = new ArrayList<>();
        for (int i = 0; i < this.length; i++) {
            if (this.countryDataArray[i].getContinent().equals(continent)) {
                arrayListContinent.add(this.countryDataArray[i]);
            }
        }
        return arrayListContinent.toArray(new CountryData[0]);
    }
    ...
    public CountryData[] filterMostPopulousCountries(int howMany) {
        CountryData[] arrayMostPopulousCountries = new CountryData[howMany];
        CountryData[] newArray = sortDecreasingByArea();
        for (int i = 0; i < howMany; i++) {
            arrayMostPopulousCountries[i] = newArray[i];
        }
    ...
    public CountryData[] filterLeastPopulousCountries(int howMany) {
        CountryData[] arrayLeastPopulousCountries = new CountryData[howMany];
        CountryData[] newArray = sortIncreasingByPopulation();
        for (int i = 0; i < howMany; i++) {
            arrayLeastPopulousCountries[i] = newArray[i];
        }
    ...
    public CountryData[] filterLargestAreaCountries(int howMany) {
        CountryData[] arrayLargestAreaCountries = new CountryData[howMany];
        CountryData[] newArray =sortDecreasingByArea();
        for (int i = 0; i < howMany; i++) {
            arrayLargestAreaCountries[i] = newArray[i];
        }
    ...
    public CountryData[] filterSmallestAreaCountries(int howMany) {
        CountryData[] arraySmallAreaCountries = new CountryData[howMany];
        CountryData[] newArray =sortIncreasingByArea();
        for (int i = 0; i < howMany; i++) {
            arraySmallAreaCountries[i] = newArray[i];
        }
        return arraySmallAreaCountries;
    }

    // Lọc ra howMany nước có GDP cao nhất.
    public CountryData[] filterHighestGdpCountries(int howMany) {
        CountryData[] arrayHighestGdpCountries = new CountryData[howMany];
        CountryData[] newArray =sortDecreasingByGdp();
        for (int i = 0; i < howMany; i++) {
            arrayHighestGdpCountries[i] = newArray[i];
        }
    ...
    public CountryData[] filterLowestGdpCountries(int howMany) {
        CountryData[] arrayLowestGdpCountries = new CountryData[howMany];
        CountryData[] newArray =sortIncreasingByGdp();
        for (int i = 0; i < howMany; i++) {
            arrayLowestGdpCountries[i] = newArray[i];
        }
        return arrayLowestGdpCountries;
    }
